...
from yolo_v3.yolov3_model_static import yolo_eval
from yolo_v3.yolo import letterbox_image
...
class YOLO(object):
    def __init__(self):
        ...
        self.model_path = os.path.join(path + '/weight/yolo_spp.h5')
        self.anchors_path = os.path.join(path + '/weight/yolo_anchors.txt')
        self.classes_path = os.path.join(path + '/weight/coco_classes.txt')
        # 具体参数可实验后进行调整
        self.score = 0.6  # 0.8
        self.iou = 0.6
    ...
    def generate(self):
        ...
        weights = self.yolo_model.get_weights()
        print(len(weights))
        print(type(weights))
        for v in weights:
            print(v.shape)
